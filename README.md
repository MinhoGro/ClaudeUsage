...
主数据源是 **Anthropic 官方用量接口** `api.anthropic.com/api/oauth/usage`，使用 **Claude Code 自己的登录令牌**（从钥匙串 `Claude Code-credentials` 或 `~/.claude/.credentials.json` 读取，**只读**，绝不修改、续期或上传）。

- 前提：终端里 `claude` 已用订阅账号登录。首次读钥匙串时 macOS 会弹授权窗，点「始终允许」。
